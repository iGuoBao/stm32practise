...
void BASIC_TIM4_Config(void)
{
	...
	TIM_TimeBaseInitTypeDef TIM_TimeBaseStructure;
	// TIM4配置 100ms
	TIM_TimeBaseStructure.TIM_Period = 1000 -1;					// 自动重装值   PER = 1 000 000 / HZ
	TIM_TimeBaseStructure.TIM_Prescaler = 72*100 -1;				// 预分频系数  1440hz 估值
	TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up;		// 向上计数
	TIM_TimeBaseInit(BASIC_TIM4, &TIM_TimeBaseStructure); 
	
	TIM_ITConfig(BASIC_TIM4,TIM_IT_Update,ENABLE);	  	// 开启溢出中断
