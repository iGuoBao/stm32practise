...
u32 LCD_RGBColor_Change(u16 color)
{
	...
	r=(color>>11)&0x1f;					// 输入16位颜色  
	g=(color>>5)&0x3f;
	b=color&0x1f;
	
	return ((r<<13)|(g<<6)|(b<<1));		// 返回32位颜色
}
...
#ifdef TFTLCD_ILI9488
	LCD_WriteCmd(0x2A);				// 地址是16位 需两次 （不确定
	LCD_WriteData(sx/256);   
	LCD_WriteData(sx%256); 	 
	LCD_WriteData(width/256); 
	LCD_WriteData(width%256);
...
//	printf("r=%x\r\n",r);
	...
	LCD_WriteCmd(0X2e);
	r=LCD_ReadData();								//dummy Read     		 				    
	r=LCD_ReadData();								//实际坐标颜色	    16位 所以两次下来才是
	b=r>>8;
	g=r<<8;
	r=g+b;
	//printf("r=%X\r\n",r);	
...
void TFTLCD_GPIO_Init(void)
{
	...
	RCC_APB2PeriphClockCmd(LCD_GPIO_RCC,ENABLE);	// 使能PORTD,E,G时钟
	
 	// PORTD复用推挽输出  
	GPIO_InitStructure.GPIO_Pin = LCD_GPIO_GROUP1_PINS;		 // PORTD复用推挽输出  
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP; 			 // 复用推挽输出   
 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	GPIO_Init(LCD_GPIO_GROUP1_PORT, &GPIO_InitStructure); 
  	 
	// PORTE复用推挽输出  
	GPIO_InitStructure.GPIO_Pin = LCD_GPIO_GROUP2_PINS;		// PORTD复用推挽输出  
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP; 		  // 复用推挽输出   
 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	GPIO_Init(LCD_GPIO_GROUP2_PORT, &GPIO_InitStructure);    	    	 											 

  // PORTG12复用推挽输出 A10	
	GPIO_InitStructure.GPIO_Pin = LCD_GPIO_GROUP3_PINS;	 	// PORTG复用推挽输出  
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP; 		 	// 复用推挽输出   
 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	GPIO_Init(LCD_GPIO_GROUP3_PORT, &GPIO_InitStructure);  
	
	// 背光控制管脚初始化
	GPIO_InitStructure.GPIO_Pin = LCD_GPIO_GROUP4_PINS;		// PB0 推挽输出 背光
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 	// 推挽输出
 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	GPIO_Init(LCD_GPIO_GROUP4_PORT, &GPIO_InitStructure);
	// LCD_LED=1;				// 点亮背光
